...
export const createPublishRoute = (db: PrismaClient, authDep: AuthDependency) => {
    ...
    publish.post("/posts/:postId/request", requireAuth, requireRole("AUTHOR", "ADMIN"), async (c) => {
        ...
        const existingRequest = await db.publishRequest.findFirst({
            where: {
                postId,
                status: "PENDING",
            },
        ...
        let publishRequest;
        try {
            publishRequest = await db.$transaction(async (tx) => {
    ...
    publish.get("/requests", requireAuth, requireRole("ADMIN"), async (c) => {
        ...
        const requests = await db.publishRequest.findMany({
            where,
            include: {
                post: {
                    select: {
...
export default createPublishRoute(prisma, auth);
